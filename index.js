...
// REDIS - Updated to fix the "NOPERM" info command warning
...
async function getAccessToken() {
    try {
        ...
        console.error("Auth Error (Check your Refresh Token):", e.response?.data || e.message);
        return null;
    }
}

// --- 2. LOGIN & CALLBACK ROUTES ---
...
async function poll() {
    ...
    const token = await getAccessToken();
    if (!token) { console.log("Poll fail: No valid token"); return; }
    try {
        ...
        if (newIds.length > 0) {
            console.log(`Found ${newIds.length} new activities. Adding to queue.`);
            await addToQueue(newIds);
        }
    } catch (e) { console.error("Poll fail:", e.message); }
}
...
async function fireKudos() {
    ...
    console.log(`Firing kudos for ${items.length} activities...`);
...
app.get('/stats', async (req, res) => {
    ...
    res.send(`<!DOCTYPE html><html><head><meta name="viewport" content="width=device-width, initial-scale=1"><style>body{font-family:sans-serif;background:#121212;color:white;text-align:center;padding:20px;}.card{background:#1e1e1e;padding:15px;border-radius:12px;border:1px solid #333;margin:10px;display:inline-block;min-width:120px;}.value{font-size:24px;color:#fc4c02;font-weight:bold;}button{background:transparent;color:#fc4c02;border:1px solid #fc4c02;padding:8px;cursor:pointer;border-radius:5px;font-size:12px;}</style><meta http-equiv="refresh" content="30"></head><body><h2 style="color:#fc4c02;">🧡 AutoKudos</h2><div><div class="card">SENT<div class="value">${total}</div></div><div class="card">QUEUE<div class="value">${qCount}</div></div></div><div style="max-width:400px;margin:20px auto;text-align:left;background:#1e1e1e;padding:20px;border-radius:12px;"><div style="display:flex;justify-content:space-between;"><span>Trawl: ${Math.floor(nextP/60000)}m</span><form action="/trawl" method="POST"><button>TRAWL</button></form></div><hr style="border:#333 1px solid;"><div style="display:flex;justify-content:space-between;"><span>Last Fire: ${lastFmt}</span><form action="/fire" method="POST"><button>FIRE</button></form></div></div></body></html>`);
});
